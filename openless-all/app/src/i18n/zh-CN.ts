...
export const zhCN = {
  ...
  settings: {
    ...
    recording: {
      ...
      macosNewlineModeDesc: '自动会在已知终端应用中使用 Line Feed（U+000A / Ctrl+J），其他应用使用 Shift+Return；Return 会直接发送。',
    ...
    providers: {
      ...
      messagesThinkingHint: "旧模型或兼容网关可能需要固定预算；思考预算必须小于最大输出。开启思考时不发送温度参数。",
      ...
      llmProtocolHeaderConflict: "Messages 已自动设置鉴权和版本请求头，请移除额外 Headers 中的 x-api-key 和 anthropic-version。",
      llmStreamError: "服务端返回流式错误，请检查模型和请求参数。",
      saveProtocol: "保存协议设置",
      thinkingModeHint: "按所选请求格式和模型支持的参数启用、关闭或降低思考，不向提示词注入控制指令。",
