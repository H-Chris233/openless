...
export const zhCN = {
  ...
  settings: {
    ...
    recording: {
      ...
      allowNonTsfFallbackDesc: 'Windows：TSF 失败后允许继续用剪贴板粘贴或 Unicode SendInput 兜底。',
      windowsNonTsfFallbackModeLabel: '非 TSF 兜底方式',
      windowsNonTsfFallbackModeDesc: '默认用剪贴板粘贴，避免长文本卡顿；如果目标应用会吞粘贴快捷键，可改用 Unicode SendInput。',
      windowsNonTsfFallbackModeClipboardPaste: '剪贴板 + 粘贴快捷键（默认）',
      windowsNonTsfFallbackModeUnicodeKeystrokes: 'Unicode SendInput（兼容吞粘贴的应用）',
      ...
      autoUpdateCheckDesc: '主窗口启动 + 后台每 60 分钟检查云端新版本。关闭后仅"关于"的手动按钮可用。',
      ...
      marketplaceDevLoginDesc: 'dev 模式用 GitHub 用户名标识上传者；空时不能上传 / 点赞。后期接入 OAuth 后此字段废弃。',
      startupAtBoot: '开机自启',
