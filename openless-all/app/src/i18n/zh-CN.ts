...
export const zhCN = {
    ...
    qa: {
        ...
        emptyDesc:
            "在任意 app 选中一段文字后，按一次 {{recordHotkey}} 开始录音，再按一次结束并提交。回答会显示在这里，可以连续多轮追问。",
    ...
    marketplace: {
        ...
        modal: {
            ...
            notLoggedIn:
                "未登录 —— 去 Settings → 录音 → 风格市场 填一个发布者名",
    ...
    onboarding: {
        ...
        hotkeyTitle: "全局快捷键",
        accessibilityDesc:
            "用于监听全局快捷键（默认 {{trigger}}）并把识别结果写入光标位置。",
        ...
        accessibilityHint:
            "授权后必须**完全退出 OpenLess** 再重新打开（macOS TCC 规则）。",
        footerHint:
    ...
    vocab: {
        ...
        corrections: {
            ...
            empty: "还没有纠正规则。",
            invalid:
                "仅支持字面替换，或一个 {num} 通配数字的规则，例如 {num}粒 → {num}例。",
            tipDisabled: "点击禁用此规则",
            tipEnabled: "点击启用此规则",
            removeAria: "删除纠正规则",
        },
    ...
    style: {
        ...
        customPromptPlaceholder:
            "可选，追加到这个风格的内置 system prompt 末尾。",
        customPromptHint:
    ...
    selectionAsk: {
        ...
        howto: {
            ...
            privacyTitle: "隐私",
            privacyDesc:
                "选中文本仅存于内存，关闭浮窗即销毁。超 4000 字符自动截断。",
        },
    },
    settings: {
        kicker: "SETTINGS",
        ...
        providers: {
            ...
            volcengineMappingNote:
                "Secret Key 当前无需填写。Resource ID 默认使用 volc.bigasr.sauc.duration。",
            localAsrActiveNotice:
                "当前已启用「{{name}}」，可在「高级」中切换或禁用。",
            localAsrTakeoverHint: "启动「{{name}}」后，ASR 提供商将被接管。",
            asrProviderTakenOver: "ASR 提供商已被接管",
            localAsrHint:
                "在本机运行，无需 API Key。从 HuggingFace 下载模型即可使用。",
            foundryLocalAsrHint:
                "在本机运行，无需 ASR API Key。首次使用需下载运行组件和模型。",
            localAsrPerformanceWarning:
                "本地推理比云端慢，中文准确率可能更低。适合离线或隐私敏感场景。",
            ...
            bailianVocabularyIdNote:
                "如已在百炼创建热词表，可填写 vocab-...；留空则不下发热词。",
            appIdLabel: "App ID（应用 ID）",
            accessKeyLabel: "Access Key",
            resourceIdLabel: "资源 ID",
            toolsLabel: "连接检查",
            toolsDesc:
                "先保存上方配置，再验证当前模型连通性或拉取模型；失败时仍可手动填写模型 ID。",
            validate: "验证",
            validating: "验证中…",
            fetchModels: "拉取模型",
            loadingModels: "拉取模型中…",
            ...
            endpointMustUseHttps:
                "Endpoint 必须使用 HTTPS（本地 localhost/127.0.0.1 测试除外）。",
        ...
        permissions: {
            title: "权限",
            descAcc:
                "OpenLess 需要以下系统权限。授权后通常要完全退出 App 重启一次才生效。",
            descNoAcc:
    ...
    modal: {
        ...
        account: {
            ...
            loginSync: "登录 / 同步",
            footer: "默认完全本地运行。登录后可跨设备同步词汇表与风格预设。",
        },
        personalize: {
            appearance: "外观",
            ...
            blur: "毛玻璃强度",
            blurDesc:
                "影响窗口内层 backdrop-filter 强度（macOS 系统磨砂层无法运行时调）。",
            startupOpen: "启动时打开",
            startupOverview: "概览",
            startupLast: "上次位置",
            startupAtBoot: "开机自启",
    ...
    localAsr: {
        ...
        qwenUnavailableOnWindows:
            "Windows 暂不支持 Qwen3-ASR，请使用上方 Foundry Local Whisper。",
        foundryTitle: "Windows Foundry Local Whisper",
        foundryDesc:
            "在本机识别语音，无需 ASR API Key。首次使用需下载运行组件和模型。",
        ...
        sherpaDesc:
            "Windows 使用 sherpa-onnx 在本机离线批量识别，无需 ASR API Key。",
        ...
        testActual: "识别",
        testStats:
            "音频时长 {{audio}}s · 加载 {{load}}s · 推理 {{transcribe}}s · 后端 {{backend}}",
        testFailed: "测试失败",
        ...
        keepLoadedDesc:
            "决定 Qwen3-ASR 用完后多久从内存释放，避免长期占用内存。",
