...
export const zhTW: typeof zhCN = {
  ...
  settings: {
    ...
    recording: {
      ...
      macosNewlineModeDesc: '自動會在已知終端應用中使用 Line Feed（U+000A / Ctrl+J），其他應用使用 Shift+Return；Return 會直接發送。',
      ...
      windowsShowOpenlessInKeyboardListDesc: '關閉後 Win+Space 切換輸入法時不會出現 OpenLess；SendInput 與剪貼簿貼上插入不受影響。重新開啟本項可恢復顯示。',
    ...
    providers: {
      ...
      tencentCloudNote: '使用騰訊雲「語音辨識」服務的 API 金鑰。預設 Hy-ASR-3.0-preview 支援中英與 20 種方言；Preview 僅支援 60 秒內的 16kHz 單聲道 PCM，暫不支援上下文或熱詞增強。',
      tencentTokenHubNote: '僅顯示目前在線的語言模型。部分模型始終啟用思考；關閉思考開關時將沿用該模型的固定行為。',
      ...
      messagesThinkingHint: "舊模型或相容閘道可能需要固定預算；思考預算必須小於最大輸出。開啟思考時不傳送溫度參數。",
      ...
      saveProtocol: "儲存協定設定",
      thinkingModeHint: "依所選請求格式與模型支援的參數啟用、關閉或降低思考，不在提示詞注入控制指令。",
