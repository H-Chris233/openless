...
export const ko: typeof zhCN = {
  ...
  marketplace: {
    kicker: 'MARKETPLACE',
    title: '스타일 팩 마켓',
    desc: '커뮤니티 스타일 팩 둘러보기, 원클릭 설치, 좋아요, 직접 업로드.',
  ...
  settings: {
    ...
    recording: {
      ...
      allowNonTsfFallbackDesc: 'Windows: TSF 입력이 실패하면 클립보드 붙여넣기 또는 Unicode SendInput으로 폴백합니다.',
      windowsNonTsfFallbackModeLabel: '비 TSF 폴백 방식',
      windowsNonTsfFallbackModeDesc: '기본값은 긴 텍스트 지연을 피하기 위해 클립보드 붙여넣기를 사용합니다. 붙여넣기 단축키를 무시하는 앱은 Unicode SendInput으로 전환하세요.',
      windowsNonTsfFallbackModeClipboardPaste: '클립보드 + 붙여넣기 단축키 (기본값)',
      windowsNonTsfFallbackModeUnicodeKeystrokes: 'Unicode SendInput (붙여넣기 차단 앱용)',
      ...
      historyMaxEntriesDesc: '로컬에 보관할 최근 세션 수. 비워두면 200. 범위 5–200. 초과 시 오래된 것부터 삭제.',
      ...
      recordAudioForDebugDesc: '켜면 각 세션의 마이크 원본을 wav로 저장하여 마이크 감도 / ASR 오인식 진단에 사용합니다. 음성이 평문으로 로컬에 저장되며 기록 보관 기간과 동일한 정리 규칙을 따릅니다.',
      audioRecordingMaxEntriesLabel: '원본 녹음 보관 개수',
      audioRecordingMaxEntriesDesc: '로컬에 보관할 최근 wav 개수. 비워두면 200. 범위 1–200. 초과 시 오래된 것부터 삭제. 텍스트 기록 개수와 독립.',
      ...
      autoUpdateCheckDesc: '메인 창 시작 시와 60분마다 새 버전을 확인합니다. 끄면 "정보" 패널의 수동 버튼만 동작합니다.',
      marketplaceGroupTitle: '스타일 팩 마켓플레이스',
      marketplaceBaseUrlLabel: '백엔드 URL',
      marketplaceBaseUrlDesc: '마켓플레이스 백엔드 HTTP URL. 비워두면 로컬 개발 http://127.0.0.1:8090; 프로덕션은 https://api.<도메인>.',
      marketplaceDevLoginLabel: 'GitHub 로그인 이름 (업로드 ID)',
      marketplaceDevLoginDesc: 'dev 모드에서는 GitHub 로그인 이름으로 업로더 식별. 비워두면 업로드/좋아요 불가.',
