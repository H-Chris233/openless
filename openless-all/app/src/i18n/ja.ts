...
export const ja: typeof zhCN = {
  ...
  settings: {
    ...
    codingAgent: {
      title: '音声 Agent（クイック Agent）',
      desc: 'グローバルホットキーで Claude Code（または OpenCode）を直接起動してクイックタスクを実行。デフォルト無効、ここで設定。',
      enable: '音声 Agent を有効化',
      comingSoonNote: '設定はすぐ保存されます。ホットキー起動と実行フローは今後のバージョンで対応。',
      hotkeyHint: '2 つのホットキーは「一般 → ショートカット」で設定。',
