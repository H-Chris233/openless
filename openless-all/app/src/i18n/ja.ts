...
export const ja: typeof zhCN = {
  ...
  marketplace: {
    kicker: 'MARKETPLACE',
    title: 'スタイルパック マーケット',
    desc: 'コミュニティのスタイルパックを閲覧・ワンクリックでインストール・いいね・自分のパックを共有。',
  ...
  settings: {
    ...
    recording: {
      ...
      allowNonTsfFallbackDesc: 'Windows：TSF 入力が失敗した時に、クリップボード貼り付けまたは Unicode SendInput でフォールバックします。',
      windowsNonTsfFallbackModeLabel: '非 TSF フォールバック方式',
      windowsNonTsfFallbackModeDesc: '既定では長文の停止を避けるためクリップボード貼り付けを使います。貼り付けショートカットを無視するアプリでは Unicode SendInput に切り替えてください。',
      windowsNonTsfFallbackModeClipboardPaste: 'クリップボード + 貼り付けショートカット（既定）',
      windowsNonTsfFallbackModeUnicodeKeystrokes: 'Unicode SendInput（貼り付けを無視するアプリ向け）',
      ...
      autoUpdateCheckDesc: 'メインウィンドウ起動時と 60 分ごとに新バージョンを確認します。オフ時は「バージョン情報」内の手動ボタンのみ有効。',
      ...
      marketplaceDevLoginDesc: 'dev モードでは GitHub ログイン名でアップロード者を識別。空時はアップロード/いいね不可。',
