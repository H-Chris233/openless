...
//!   不再付握手成本。
//! - `send_with_retry`：对「连接 / 超时 / 请求未送出」这三类传输层失败做指数退避
//!   重试。这类失败发生在请求送达服务端之前，重试是幂等安全的。HTTP 4xx/5xx 不在
//!   重试范围 —— 服务端已经应答，状态码交给调用方判断。
...
/// 发送请求，对传输层失败（连接重置 / 超时 / 请求未送出）做指数退避重试。
///
/// `make` 每次尝试都重新构造 `RequestBuilder`（`send()` 会消耗它）。只重试
/// `is_connect()` / `is_timeout()` / `is_request()` —— 这三类失败时请求尚未送达
/// 服务端，重试幂等安全。拿到任意 HTTP 响应（含 4xx/5xx）即返回，状态码由调用方
/// 自行判断。
