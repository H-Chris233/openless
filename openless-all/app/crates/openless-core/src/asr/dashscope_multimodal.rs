...
pub use crate::provider_rules::DashScopeBatchProtocol;

pub fn protocol_for_model(model: &str) -> Option<DashScopeBatchProtocol> {
    crate::provider_rules::dashscope_batch_protocol_for_model(model)
}
...
impl super::AudioConsumer for DashScopeMultimodalASR {
...
mod tests {
    ...
    fn extracts_text_from_async_result_documents() {
        ...
        assert_eq!(
            extract_async_transcript_text(&funasr).unwrap(),
            "第一句 第二句"
        );
        ...
        assert_eq!(
            extract_async_transcript_text(&qwen).unwrap(),
            "Qwen 转写结果"
        );
    }
